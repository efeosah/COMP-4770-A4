...
//#warning improve (mainly for bag) by using a Node iterator instead of ItemMultiplicities()
        ...
        [Serializable]
        class Multiplicities : CollectionValueBase<SCG.KeyValuePair<T, int>>, ICollectionValue<SCG.KeyValuePair<T, int>>
        {
            ...
            SCG.IEnumerator<SCG.KeyValuePair<T, int>> GetEnumerator(Node node, int origstamp)
            {
                if (node == null)
                {
                    yield break;
        ...
        int MaxSnapId => snapList == null ? -1 : FindLastLiveSnapShot();
        ...
        internal class Range : DirectedCollectionValueBase<T>, IDirectedCollectionValue<T>
        {
            int stamp;
            readonly int size;
            readonly TreeBag<T> basis;

            readonly T lowend, highend;

            readonly bool haslowend, hashighend;
