...
namespace C5
{
    ...
    public class HashedArrayList<T> : ArrayBase<T>, IList<T>, SCG.IList<T>
    {
        ...
        /// </summary>
        int UnderlyingSize => (underlying ?? this).size;
        ...
        readonly HashSet<SCG.KeyValuePair<T, int>> itemIndex;
        #endregion
        #region Events
        ...
        {
            SCG.KeyValuePair<T, int> p = new SCG.KeyValuePair<T, int>(item, offsetField + i);
        ...
        public virtual T this[int index]
        {
            ...
            set
            {
                ...
                SCG.KeyValuePair<T, int> p = new SCG.KeyValuePair<T, int>(value, index);
        ...
        public virtual void InsertAll(int index, SCG.IEnumerable<T> items)
        {
            ...
            try
            {
                ...
                {
                    SCG.KeyValuePair<T, int> p = new SCG.KeyValuePair<T, int>(item, i);
                    if (itemIndex.FindOrAdd(ref p))
                    {
                        throw new DuplicateNotAllowedException("Item already in indexed list");
                    }
        ...
        IList<V> Map<V>(Func<T, V> mapper, HashedArrayList<V> res)
        {
            int stamp = this.stamp;
            if (size > 0)
            {
                for (int i = 0; i < size; i++)
                {
                    ...
                    ModifyCheck(stamp);
                    SCG.KeyValuePair<V, int> p = new SCG.KeyValuePair<V, int>(mappeditem, i);
        ...
        {
            UpdateCheck();
            SCG.KeyValuePair<T, int> p = new SCG.KeyValuePair<T, int>(item, size + offsetField);
            if (itemIndex.FindOrAdd(ref p))
            {
                return false;
            }
